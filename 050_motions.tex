...
に「三角関数の合成」(数学の教科書を参照せよ)を適用すれば, \eref{eq:spring_Asinphi}の形に
...
\eref{eq:mromegasq_vec}を満たすような何らかの力が質点に働いていると, その質点は等速円運動をする。
\eref{eq:mromegasq_vec}を「向心力」\index{こうしんりょく@向心力}と言う。というのも, 
この${\bf F}$は円運動の中心から質点へのベクトル${\bf r}$とは真逆の向き, つまり質点
から「中心への向き」だからである。

\begin{q}\label{q:circular_motion} 以下の等速円運動（厳密にはそうとは言えないものもあるが, それも近似的に等速円運動とみなす）において, 向心力は何によって実現されているか?
\begin{enumerate}
\item 太陽の周りを地球がまわること。
...
「遠心力」は\pref{sec:inertforce_rotate}で学ぶまで不用意に使わないようにしよう。\\
...
\begin{exq}　以下の問に答えよ。ただし「遠心力」という言葉を使ってはならない（というか, 本来, 使う必要は無いのに使おうとする人が多い...泣）。
\begin{enumerate}
...
\item 略（運動方程式${\bf F}=m{\bf a}$に\eref{eq:circle_motion_a}を代入）。
\item 略（\eref{eq:mromegasq_vec}, \eref{eq:circle_motion_vxvy}のそれぞれの大きさを求める）。
\item 略（\eref{eq:mromegasq}と\eref{eq:circle_motion_v_romega}で$\omega$を消去）。
