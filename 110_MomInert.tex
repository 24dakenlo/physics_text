...
\begin{faq}{\small\textgt{物体の運動をここまで数式で予測できるなんてすごい
と思いますが, これを実際に自分で思いつけるかというと, できる気がしません。
ますます私には物理は無理ってかんじです。} ... そう思うのは当然です。
...
\begin{enumerate}
\item 略。
\item \eref{eq:mominert_def}より, $I$の単位は$m_kr_k^2$の単位なので, kg~m$^2$
\end{enumerate}
...
慣性モーメントのもともとの定義, つまり\eref{eq:mominert_def}では, 
...
\item 略（\eref{eq:mominert_wheel_slope3}を$v=$の形に式変形すればよい）。
\item 略（\eref{eq:mominert_wheel_slope_v}に$I=0$を代入するだけ）。
\item \eref{eq:mominert_ring}を\eref{eq:mominert_wheel_slope_v}に代入して与式を得る。
\item \eref{eq:mominert_disc_horizon}を\eref{eq:mominert_wheel_slope_v}に代入して, 
\begin{eqnarray}
v=\sqrt{\frac{4g\,h}{3}}
\end{eqnarray}
\end{enumerate}
...
\noindent{\textbf{答}}\ref{q:mominert_molecule}
\begin{enumerate}
...
\end{eqnarray*}
この式から与式を得る。
\item 略(\eref{q:mominert_pandulum6}を使って\eref{q:mominert_pandulum5}から$I$を
...
\item \eref{q:mominert_pandulum9}を$g=$の形に式変形すれば与式を得る。
\end{enumerate}
\end{q}
\mv
