...
質点の運動が3次元的であるならば, \eref{eq:kineticEnergy_again}の
...
数学リメディアル教材を参照せよ。}。\eref{eq:work_Fdotdr}は, \eref{eq:work}を3次元に拡張した
式でもある。
...
\begin{faq}{\small\textgt{何ですか!? この積分!? こんなの見たことありません。
普通の積分は, $\int$なんちゃら$dx$とか$\int$なんちゃら$dt$みたいに, $d$が
つくのは$x$や$t$などのひとつの変数です。でも, これは$d{\bf r}$って, ベクトルに
$d$がついてます。しかも内積!? わけわかりません} ... 
...
\end{eqnarray}
で定義される関数$U({\bf r})$を, 「\textgt{ポテンシャルエネルギー}」という。
...
\end{eqnarray}
である。\eref{eq:3D_cons_F_ma}に\eref{eq:3D_cons_dr_vdt}を辺々, 内積すると, 
\begin{eqnarray}
{\bf F}\bullet d{\bf r}=m\frac{d{\bf v}}{dt}\bullet {\bf v}dt\label{eq:3D_WandT000}
\end{eqnarray}
となる。これはちょうど, \eref{eq:WandT000}を3次元に拡張した式だ。\mv
...
\end{eqnarray} 
%ヒント:数学リメディアル教材 5.7節 または12.2節。

この近似式が成り立つとして, 以下の小問に答えよ：

\item $\omega=\sqrt{g/l}$とすると\eref{eq:furiko_energy5}は次式になることを示せ：
\begin{eqnarray} 
...
\item 略。
\item 略(\eref{eq:furiko_energy6}の左辺と右辺に代入して, それらが等しくなることを示せばよい)。
...
\item \eref{eq:furiko_energy_ans5}より, $\tau$は$\sqrt{l}$に比例するから, $\tau$を半分にするには, $\sqrt{l}$を半分にすればよい。従って$l$を1/4倍にすればよい。
\end{enumerate}
\vspace{0.2cm}
...
\begin{enumerate}
\item \eref{eq:furiko_energy_ans5}を変形して$g=$の式にすると, 
\begin{eqnarray}g=\frac{4\pi^2l}{\tau^2}\end{eqnarray}
\item 月面では重力加速度が地表の1/6倍になる。\eref{eq:furiko_energy_ans5}より, 
